...
/**
 * Функція `findIndexByProperty` знаходить індекс першого об'єкта в масиві, властивість 'name'
...
function findLastEvenNumber(arr) {
  ...
  const lastEventNumber = arr.findLast( (element) => element % 2 === 0);
  if(lastEventNumber) {
    return lastEventNumber;
  } 
  return console.log("парних чисед не знайдено") // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
/**
 * Функція `getSubArrayAndConvertToString` отримує підмасив від заданого індексу до кінця масиву 
 * //і конвертує його в рядок.
...
function getPositiveArrayValues(arr) {
  ...
  const positiveArrayValues = Array.from(arr.values()).filter((element) => element > 0);
  if (positiveArrayValues) {
...
/**
 * Функція `removeAndDouble` видаляє елементи з масиву починаючи з заданого індексу і подвоює 
 * залишені елементи.
...
/**
 * Функція `flattenSquareAndFilter` вирівнює масив, підносить кожен елемент до квадрату і фільтрує 
 * по заданому порогу.
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const arraySlice = arr.slice(-n);
  const sum =  arraySlice.reduceRight((accumulator, currentValue) => accumulator + currentValue);

  arraySlice.forEach((element) => console.log(element));
  return arraySlice, sum;
...
/**
 * Функція `wordLengths` приймає масив слів та повертає новий масив, де кожний елемент - це довжина
 *  відповідного слова.
...
/**
 * Функція `flattenArray` приймає масив (вкладені масиви допустимі) і повертає новий одновимірний 
 * масив з усіма елементами.
...
/**
 * Функція `expandArrayByN` приймає масив чисел і число n, та повертає новий масив, де кожне число з початкового масиву
...
function capitalizeWords(arr) {
  ...
  const capitalizedArray = arr.map((element) => element[0].toUpperCase() + element.slice(1));
...
function calculateTotalPrice(arr) {
  ...
  const totalPrice = arr.reduce((accumulator, currentValue) => accumulator + currentValue.price, 0);
  return totalPrice;
